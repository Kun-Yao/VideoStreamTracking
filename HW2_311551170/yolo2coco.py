# -*- coding: utf-8 -*-
"""
Created on Thu Oct 27 10:55:05 2022
...
def yolo2coco(arg):
    ...
    if arg.save_path == 'instances_val2017.json':
        originLabelsDir = os.path.join(root_path, 'val_labels')                                        
        originImagesDir = os.path.join(root_path, 'val')
    elif arg.save_path == "instances_train2017.json":
        originLabelsDir = os.path.join(root_path, 'train_labels')                                        
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        # 读取图像的宽和高
        if arg.save_path == 'instances_val2017.json':
            im = cv2.imread(os.path.join(root_path, 'val/') + index)
        elif arg.save_path == "instances_train2017.json":
